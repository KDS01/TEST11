...
public class SingletornDAO {
	private ConnectionMaker maker;//오로지 커넥션만을 위한 커넥션메이커 클래스를 새로 만들어서, 기능을 분리한다.
	private Connection con;
	public static SingletornDAO INSTANCE;
	public SingletornDAO(ConnectionMaker maker) {
		this.maker=maker;
	}
	public static SingletornDAO getInstance() {
		return INSTANCE;
	}
	public static SingletornDAO getInstance(ConnectionMaker maker) {
		if(INSTANCE==null)INSTANCE = new SingletornDAO(maker);
		return INSTANCE;
	}
